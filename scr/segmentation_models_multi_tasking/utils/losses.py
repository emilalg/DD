...
class FocalTverskyLossPlusPlus(base.Loss):  # under construction

    def __init__(self, alpha=0.3, eps=1e-5, beta=0.7, gamma=0.75, activation=None, ignore_channels=None):
        super().__init__()
        self.eps = eps
        self.alpha = alpha
        self.beta = beta
        self.gamma_ftl = gamma  # Renamed to avoid confusion with gamma in DSC++
        self.activation = Activation(activation)
        self.ignore_channels = ignore_channels
    ...
    def forward(self, y_pr, y_gt):
        ...
        for c in range(C):
            ...
            # Modifying the false positives and negatives according to DSC++
            ...
            class_loss = 1 - numerator / denominator
            loss += torch.pow(class_loss, self.gamma_ftl)  # Applying the focal component
